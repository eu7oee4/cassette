...
def one_turn_hint(kind: str = "chat") -> str:
    ...
    if kind == "chat_session":
        # 机主拍板（08-30）：session 路把「这轮那轮」的解释整段删掉，只留机制。
        # **09-04（§14.2）这一支从每轮注入挪进了系统提示**，同时删掉用法半句——
        # 用法归工具 schema（那儿写得比这儿全），这里只剩**纪律**。
        # 原来的形状是「系统提示一份 + 每轮注入一份」，同一条规矩付两遍，而且
        # 两处措辞不一样时他照更近的那条写（pipeline.py:318 那个坑）。
        return ("【要提一件还没做的事，二选一：\n"
                "① 这一轮里就做掉，做完了再开口——工具都在你手上，回复里报结果，"
                "别报打算；\n"
...
def pending_todo_block(char_id: Optional[str] = None, kind: str = "chat") -> str:
    ...
    两种形态，按「到点没到」分：
    · **还没到点**（聊天里看见它）：报钟点。2026-09-01 补的，欠的账是这次：08-31 20:59
      他自己钉了 22:59 的钟，21:24 在聊天里又写了一句 [[next_wake:8小时]]——schedule
      只有一个 next_wake_at 槽，finalize_chat_reply 无条件覆盖，22:59 当场作废，
      从外面看像「闹钟没响」。在此之前这块只递「留的活」不递钟点：钟没配待办时整块
      都不出现，他重钉的时候手上一个字都没有，等于闭着眼睛顶掉自己的承诺。
    · **已经到点**（scheduled 醒来那轮走到这儿，钟还没被 finish_wake_turn 消费）：
      现在就是那个「下一轮」。没留活就不说话——一个空钟没什么可交代的。

    **两套措辞，按 kind 分（§14.2，09-04）**：

    · `kind != "wake"`（SDK 常驻路）＝**一行状态行，纯知觉材料**：只报「手上有什么」，
      一句规矩都不带。规矩挪进了系统提示付一次（one_turn_hint("chat_session")），
      「钟只有一个」归工具 schema 和回执——回执是在他动手那一刻报的，比任何静态句子准。
      原来那三句「这一轮再调一次…是把这个钟挪到新时间」每轮都注，是同一条规矩的第三份。
    · `kind == "wake"`（-p 熄火回退路 / cohabit 老路）＝**原文照旧**。那条路没有系统
      提示可付，规矩只能跟着注入走；顺手改它等于在改一条正在当降级兜底用的路。

    ⚠️ 这里**只管怎么写，不管什么时候写**——「状态行只在他记忆里没有这个钟时出现」
    是 §14.3，没做，所以眼下照旧每轮都出。"""
    try:
    ...
    if at is not None and at > time.time():
        ...
        when = f"{fmt_ts(int(at))}（{fmt_gap(int(at - time.time()))}后）"
        if kind != "wake":
            return (f"【下一次醒来的闹钟：{when}——「{todo}」】" if todo
                    else f"【下一次醒来的闹钟：{when}。】")
        head = f"【你现在钉着一个钟：{when}"
        head += f"，到点要做的是：「{todo}」\n" if todo else "，没配待办。\n"
        return (head +
                "钟只有一个：这一轮再在 NEXT 那段重写一个时间，是把这个钟**挪到新时间**，"
                "不是另加一个闹钟。"
    ...
    if not todo:
        return ""
    if kind != "wake":
        # 措辞不许暗示他失约（§14.3 口径）：清槽是机制干的、不看他做没做，
        # 到点未清只意味着那次醒来没走完。
        return f"【闹钟到点了：{fmt_ts(int(at))}——「{todo}」】" if at else \
               f"【上一轮给自己留的活：「{todo}」】"
