...
def cohabit_prompt(cid: str, reasons: list[dict], settings: dict) -> str:
    ...
    # 家里有宠物时 CARRY 才提得到猫（没有就一个字不多说，别让模型惦记不存在的猫）
...
def _cohabit_wake_body(cid: str, reasons: list[dict], _chain_no: int,
    ...
    if p["move"]:
        if move_stopped:
            ...
            return result
        # CARRY 解析（政策层）：认**此刻同屋的用户或宠物**——名字或 id 都行；对不上
        # 就当没写并记日志（隔空抱、抱别的 AI 在结构上不存在）。
        # 抱人要对方愿意（2026-08-17 机主拍板）：认出用户也**不在这轮移动**——落一个
        # 邀约（offers.create：房间系统事件给在场者插话窗口 + 用户在 app 里应答），
        # 答应了才 move+carry；答应/拒绝/没反应都会以补醒回来，这轮到此为止。
        # 抱猫不用先问（PLAN_pet P0）：直接抱走，猫乐不乐意是 P1 猫引擎的事。
        carry = None
        if p["carry_raw"]:
            loc_now = world.location_of(cid)
            u_name = world.entity_name(world.USER_ID)
            pid = pets.match(p["carry_raw"])
            if p["carry_raw"] in (world.USER_ID, u_name) \
                ...
                return result
            if pid and world.location_of(pid) == loc_now:
                carry = pid
            else:
                logerr(f"cohabit CARRY 抱不了（不同屋/不认识/非用户非宠物），当没写："
                       f"{p['carry_raw']!r}")
        try:
            mv = world.move(cid, p["move"], carry=carry)
        except ValueError as e:
            # 预检和落地之间世界可能变了（猫刚好被人抱走）：退成独自移动，别整轮报废。
