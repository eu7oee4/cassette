...
## 16. 学舌：当前轮和历史轮同形（2026-09-12，抄 nostos 的定案）

- **现象**：09-07 14:02 Cassius 答完自己那句没停，接着把下一条 user 轮也写出来了：
  `user【09-07 周一 14:02】【距离上一条消息，过了 1 分钟】【回下面这条。按这句的份量…】眠眠：ipad在这里…`。
  角色标签 `user` 来自引擎缝隙学舌，框的内容来自每轮注入。
- **根因**（nostos assemble.py 的分析，照搬）：当前轮比历史轮多两样——一句「【回下面这条…】」和「眠眠：」
  前缀。模型模仿**只在当前轮出现的格式**：它每轮出现在紧贴生成边界的位置，看多了就成了「轮次开始的仪式」。
  历史里全是同一个形状，就没有仪式可学。「名字：台词」是剧本格式，剧本天然招续写。
- **改**：
  1. `chat_loop.build_injection`：只剩 【时间】【距离】（+ 钟/待办块）+ 光正文；`_stamp_times` 历史 user 轮同形
     （`【戳】\n正文`，不带名字）。assistant 轮照旧不打戳。
  2. 「按份量回」挪进系统提示付一次（`pipeline.turn_frame_hint`），并加一句「框是读数不是话，不复述、不给自己加框」。
  3. 第二道闸 `pipeline.scrub_turn_frame` 写好了、测试在，**但故意不挂**（机主拍板）：学舌原样落进聊天，
     机主看得见才知道 1+2 有没有治住；复发了再挂到 sse.translate_events / finish_wake_turn。
     挂上之后的道理：别让学舌的字节落库——落了库下一轮就是历史里的样本，自我放大。
- **没动**：-p 回退路（`pipeline.build_prompt`）历史是扁平文本、本来就带名字前缀，当前轮和历史一致，只是多一句
  「回下面这条」——那条路熄火才走，先不动。
- **代价**：系统提示变了 + 历史渲染变了 → 重启后首轮全量重铸一次，之后照旧。
- **验**：一周内看聊天里有没有再冒出 `user【…】` / 「眠眠：」续写；有就把刷子挂上。
