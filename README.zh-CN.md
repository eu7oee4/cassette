...
- **插件商店**：工具族不进主仓，各自是独立小仓，app 里一键下载/启用/卸载（后端动态挂载，开关即时生效）。第一个插件「网页工坊」：TA 能做/改自包含的 HTML 页面，你在聊天记录 → HTML 文件里点开看。registry 是写死的白名单，没有"输入 URL 安装"。
